...
export class InteractionManager {
  ...
  public setLoadingState(element: HTMLElement, loading: boolean): void {
    if (loading) {
      ...
      spinner.className =
        'loading-spinner inline-block w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin mr-2'
  ...
  private createTooltip(element: HTMLElement): void {
    ...
    tooltip.className =
      'absolute z-50 px-3 py-2 text-sm text-white bg-gray-900 rounded-lg shadow-lg opacity-0 pointer-events-none transition-opacity duration-200'
  ...
  private initializeFormEnhancements(): void {
    // Floating labels
    const floatingLabelInputs = document.querySelectorAll(
    ...
    const validatedInputs = document.querySelectorAll(
